 ...
 *
 *   GET /v1/dashboard      server.ts:283-308   → Dashboard          (dashboard.ts:154-164)
 *   GET /v1/portfolio      server.ts:310-343   → { portfolio: Tile<PortfolioView> }  (line 338)
 *   GET /v1/activity       server.ts:345-392   → a FLAT page, not a Tile  (lines 377-387)
 *   GET /v1/search         server.ts:394-417   → SearchResponse     (search.ts:56-66)
 *   GET /v1/next-actions   server.ts:419-505   → NextActions        (nextactions.ts:94-97)
 *
...
/**
 * One asset the account holds. Mirrors `Holding` in hub-api/src/portfolio.ts:39-57.
 *
 ...
 * when the oracle blinks, and the user reads that as a loss" (portfolio.ts:140-142).
 */
...
/** One bar of the allocation chart. Mirrors `AllocationRow`, portfolio.ts:60-65. */
...
/** Mirrors `PortfolioView`, portfolio.ts:67-84. */
export interface PortfolioView {
  readonly totalUsdScaled: string
  readonly totalUsd: string
  /** The OLDEST contributing observation, never the newest and never now. portfolio.ts:8-11. */
...
/** Mirrors `WalletRecord`, hub-api/src/upstreams.ts:165-180 (wallet's `GET /v1/wallets`). */
...
/** Mirrors `DepositCredit`, upstreams.ts:183-195. */
...
/** Mirrors `WithdrawalRecord`, upstreams.ts:198-224. */
...
/** Mirrors `SecurityView`, hub-api/src/dashboard.ts:99-117. */
...
/** Mirrors `SecurityAlert`, dashboard.ts:121-126. */
...
/** Mirrors `PolicyFreeze`, upstreams.ts:312-320. */
...
/** Mirrors `BillingEntitlement`, upstreams.ts:256-264. */
...
/** Mirrors `BillingSubscription`, upstreams.ts:267-275. */
...
/** Mirrors `Entitlements`, dashboard.ts:144-147. */
...
/** Mirrors `Notifications`, dashboard.ts:149-152. Permanently unavailable — see the README. */
...
/** Mirrors `ActivityRecord`, upstreams.ts:278-290 (activity's `GET /feed`). */
...
/** Mirrors `PricingRate`, upstreams.ts:298-309 (pricing's `GET /rates` — NOT `/v1/quotes`). */
...
/** Mirrors `ActionProgress`, hub-api/src/nextactions.ts:65-70. */
...
/** Mirrors `NextAction`, nextactions.ts:72-86. */
...
/** Mirrors `NextActions`, nextactions.ts:94-97. */
...
/** Mirrors `DashboardTiles`, dashboard.ts:128-142. Eleven tiles, and every one is a `Tile<T>`. */
...
/** Mirrors `Dashboard`, dashboard.ts:154-164. */
 ...
 * produce a 503 has misunderstood the design" (dashboard.ts:189-198). So a rejection from this
 ...
 * beside `status`, `reason`, `cached` and `ageMs` at the top level (server.ts:377-387). Typing it
 ...
 * be kept in step with the first for ever" (server.ts:348-357).
 */
...
/** `limit` must be a whole number between 1 and 100, or hub-api answers 400 (server.ts:570-580). */
...
export function loadActivity(
  ...
  const options: RequestOptions = {
    ...
    // (server.ts:362, 369). Sending an empty value would defeat the cache on every first load.
...
/** Mirrors `SearchResponse`, hub-api/src/search.ts:56-66. */
...
/** Mirrors `SearchGroup`, search.ts:50-54. */
...
/**
 * Search. `q` is required and non-empty, or hub-api answers 400 (server.ts:400-405), and it is
 * capped at 128 characters (server.ts:130, 403).
 */
 ...
 * (hub-api/src/upstreams.ts:21-28). The session LIST and the revoke are not composed at all —
 ...
 * the operations identity alone owns. `docs/ecosystem/03-repository-responsibilities.md:150`
 * moves the Nimbus `/account` page into this repository, which is the mandate for it.
 */

/** Mirrors `listSessions`, identity/src/sessions.ts:246-263. Active sessions only, newest first. */
...
/** `GET /sessions` — identity/src/server.ts:1077-1080. */
...
/**
 * `DELETE /sessions/:id` — identity/src/server.ts:1092-1100.
 *
...
/**
 * `DELETE /sessions` — identity/src/server.ts:1082-1090. Signs out EVERYWHERE, including here.
 *
...
/** Mirrors identity's `GET /mfa/factors` body — identity/src/server.ts:1104-1113. */
